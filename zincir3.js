...
window.onload = function() {
  ...
  function loadHabits() {
    ...
    for (const habitName in habits) {
      ...
      // Alışkanlık adını ve silme butonunu ekle
      ...
      deleteButton.addEventListener("click", function() {
        deleteHabit(habitName); // Silme fonksiyonunu çağır
      });
      habitItem.appendChild(deleteButton);
  
      habitItem.addEventListener("click", function(event) {
        if (!event.target.classList.contains("delete-habit")) { // Silme butonuna tıklanmadığında tabloyu aç
